...
export class PaymentService {
   ...
   * @returns Observable with clientSecret for Stripe Elements
   */
  createStripePaymentIntent(orderId: string, amount: number): Observable<{ clientSecret: string }> {
    return this.http.post<{ clientSecret: string }>(
      `${this.API_URL}/stripe/create-intent`,
      { orderId, amount }
    );
  }
   ...
   * @returns Observable with payment confirmation result
   */
  confirmStripePayment(clientSecret: string, paymentMethodId: string): Observable<any> {
    return this.http.post(
      `${this.API_URL}/stripe/confirm`,
